...
#
# The output is a plot, showing the number received Data packets by
# the consumer during *each* video playing back (i.e, each session).
#.....................................................................
...
if len(sys.argv) == 1 :
    print "program usage:\n\tpython session-segments.py <log-file>\n"
    sys.exit(1) 

command = "python parser.py " + sys.argv[1] + " | sort -k 14,14 -k 10,10 | awk '{print $14\",\"$10}'"
process = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE)
output, err = process.communicate()
session_list = output.split('\n')
session_list = filter(None, session_list)

seg_list = [] # each cell stores a tuple <session_id, session_sum>
current_session = "VOID"
session_sum = 0 # calculate the sum
for s in session_list:
    if current_session != s.split(',')[0]:  # new session
        if current_session != "VOID":
            # record number of segments from previous session
            seg_list.append((current_session, session_sum))
            # reset it to 0
            session_sum = 0
        try:
            session_sum  =  session_sum + int(s.split(',')[1], 10)
...
# record number of segments for the last session
...
f.writelines(["set terminal dumb\n",
              "set title 'Number of segments during each session'\n",
