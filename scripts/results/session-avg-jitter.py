...
avg_list = [] # each cell stores a tuple <session_id, session_avg_jitter>
current_session = 0
session_avg_jitter = 0
session_n_samples = 0
for s in session_list:
    ...
    if current_session != s.split(',')[0]:
        if current_session != 0:
            avg_list.append((current_session, session_avg_jitter))
        current_session = s.split(',')[0]
        session_n_samples = 1
        session_avg_jitter = float(s.split(',')[1])
        continue
    session_avg_jitter = ((session_n_samples * session_avg_jitter) + float(s.split(',')[1])) / (session_n_samples + 1)
    session_n_samples += 1
